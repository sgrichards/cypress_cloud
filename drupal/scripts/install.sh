...
if [ ! -f "/opt/drupal/web/sites/default/settings.php" ]; then
    ...
    drush --root=/opt/drupal/web si cohesion_install --db-url=mysql://root:MYSQL_ROOT_PASSWORD@database:3306/${MYSQL_DATABASE} --account-name=${DRUPAL_USER} --account-pass=${DRUPAL_PWD} -y -v
